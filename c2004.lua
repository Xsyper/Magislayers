--Magislayer Hero - Liu Bei
--Created and Scripted by Xsyper
local s,id=GetID()
function s.initial_effect(c)
	--synchro summon
	Synchro.AddProcedure(c,aux.FilterBoolFunctionEx(Card.IsSetCard,0x0309),1,1,Synchro.NonTunerEx(Card.IsSetCard,0x0309),1,99)
	c:EnableReviveLimit()
...
function s.spfilter1(c,e,tp)
	return c:IsSetCard(0x0309) and c:IsCanBeSpecialSummoned(e,0,tp,true,false) and not c:IsType(TYPE_SYNCHRO)
